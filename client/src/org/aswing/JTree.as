...
public class JTree extends Container implements Viewportable, TreeModelListener, CellEditorListener{
    ...
    public static const SINGLE_TREE_SELECTION:int = DefaultTreeSelectionModel.SINGLE_TREE_SELECTION;
    ...
    public static const CONTIGUOUS_TREE_SELECTION:int = DefaultTreeSelectionModel.CONTIGUOUS_TREE_SELECTION;

    /** Selection can contain any number of items that are not necessarily
     * contiguous. */
    public static const DISCONTIGUOUS_TREE_SELECTION:int = DefaultTreeSelectionModel.DISCONTIGUOUS_TREE_SELECTION;
    ...
    public function getExpandedDescendants(parent:TreePath):Array {
		...
		if(toggledPaths != null) {
			for(var i:int=0; i<toggledPaths.length; i++){
     ...
     *		if there is no tree
     */
    private function getPathBetweenRows(index0:int, index1:int):Array {
        ...
        if(tree != null) {
            ...
            for(var counter:int = newMinIndex; counter <= newMaxIndex; counter++){
    ...
    */
    ...
    protected function firePropertyChange(pn:String, oldValue:*, newValue:*):void{
    ...
    protected function getModelIndexsForPath(path:TreePath):Array {
		if(path != null) {
		    ...
		    for(var counter:int = 1; counter < count; counter++) {
	...
	private function removeDescendantToggledPaths(toRemove:Array):void{
		if(toRemove != null) {
			for(var i:int=0; i<toRemove.length; i++){
				...
				if(descendants != null) {
					for(var j:int=0; j<descendants.length; j++){
